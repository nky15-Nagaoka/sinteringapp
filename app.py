...
# 物理モデル重み係数ガイド
#
# sintering_core.py では、拡散・液相焼結・焼結助剤・第二相ピン止めなどを
# 「物理式そのもの」だけでなく、半定量的な重み係数で調整できるようにしています。
#
# これは、文献値のD0やQだけでは実験結果を完全には再現しにくいためです。
# Researchモードでも、材料系に応じて「どの機構が効きやすいか」を人間が調整できます。
#
# 基本的な読み方:
#   1.0 : 標準
#   0.5 : その効果を弱める
#   2.0 : その効果を強める
#   0.0 : 感度解析用にその寄与をほぼ無効化
#
# 注意:
#   重みは物理定数そのものではなく、モデル寄与の補正係数です。
#   実験データ（TMA、密度、SEM粒径など）で校正して使うのが理想です。
# =============================================================================
def render_weight_guide(p_obj=None):
    rows = [
        {
            "重み係数": "w_diffusion",
            "日本語名": "拡散全体の寄与",
            "標準値": 1.0,
            "推奨範囲": "0.2 – 3.0",
            "学生向け説明": "Ds, Db, Dvから来る緻密化全体の強さを調整します。大きくすると密度上昇が速くなります。",
            "主に効く結果": "相対密度、気孔率"
        },
        {
            "重み係数": "w_grain_growth",
            ...
            "標準値": 1.0,
            "推奨範囲": "0.0 – 3.0",
            "学生向け説明": "粒界移動による平均粒径の増加を調整します。大きくすると粒径が大きくなりやすくなります。",
            "主に効く結果": "平均粒径、硬度"
        },
        {
            "重み係数": "w_liquid_phase",
            ...
            "標準値": 1.0,
            "推奨範囲": "0.0 – 3.0",
            "学生向け説明": "Kingery型液相焼結、粒子再配列、溶解再析出の寄与を調整します。液相焼結材料では特に重要です。",
            "主に効く結果": "密度、粒成長、液相焼結速度"
        },
        {
            "重み係数": "w_sintering_aid",
            "日本語名": "焼結助剤寄与",
            "標準値": 1.0,
            "推奨範囲": "0.0 – 3.0",
            "学生向け説明": "助剤による拡散促進、液相形成、濡れ性改善の効果を調整します。大きくすると低温でも緻密化しやすくなります。",
            "主に効く結果": "密度、液相効果、拡散係数"
        },
        {
            "重み係数": "w_second_phase_pinning",
            "日本語名": "第二相ピン止め寄与",
            "標準値": 1.0,
            "推奨範囲": "0.0 – 3.0",
            "学生向け説明": "ナノ粒子や第二相が粒界移動を妨げるZenerピン止め効果を調整します。大きくすると粒成長が抑制されます。",
            "主に効く結果": "平均粒径、異常粒成長"
        },
        {
            "重み係数": "w_atmosphere",
            ...
            "標準値": 1.0,
            "推奨範囲": "0.0 – 3.0",
            "学生向け説明": "酸素分圧や水蒸気分圧による欠陥化学・粒界状態の変化を調整します。酸化物や非酸化物で効き方が変わります。",
            "主に効く結果": "拡散係数、密度"
        },
        {
            "重み係数": "w_electric_field",
            "日本語名": "電場・通電寄与",
            "標準値": 1.0,
            "推奨範囲": "0.0 – 3.0",
            "学生向け説明": "SPS/FAST/Flash焼結での電場促進やジュール発熱の寄与を調整します。通常炉焼結では0に近くても構いません。",
            "主に効く結果": "拡散係数、局所昇温、熱暴走"
        },
        {
            "重み係数": "w_closed_pore",
            "日本語名": "閉気孔化による抑制",
            "標準値": 1.0,
            "推奨範囲": "0.0 – 3.0",
            "学生向け説明": "開気孔が閉気孔になった後の緻密化減速を調整します。大きくすると後期焼結で密度上昇が止まりやすくなります。",
            "主に効く結果": "後期密度、残留気孔"
        },
    ]
    df = pd.DataFrame(rows)
    if p_obj is not None:
        df.insert(2, "現在値", [getattr(p_obj, r["重み係数"], 1.0) for r in rows])
    st.markdown("#### 物理モデル重み係数一覧")
    st.caption("1.0が標準です。Researchモードでも材料系に応じて調整できます。実験データで校正すると精度向上に有効です。")
    st.dataframe(df, use_container_width=True, hide_index=True)
...
# =============================================================================
# 物理モデル重み係数（Researchモードでも調整可能）
#
# ここでは、拡散・液相・助剤・第二相・雰囲気などの寄与を
# 物理的に意味のある範囲で手動調整できます。
#
# 1.0 が標準値です。
# 0.5 は効果を弱める、2.0 は効果を強める、0.0 は感度解析用にほぼ無効化する意味です。
#
# 実験データがある場合は、TMA収縮曲線・実測密度・SEM粒径に合うように
# これらの重みを調整すると、材料ごとの予測精度が上がります。
# =============================================================================
with st.sidebar.expander("物理モデル重み係数（Researchモードでも調整可能）"):
    st.caption("1.0が標準です。0.5で弱め、2.0で強めます。0.0は感度解析用です。")

    p.w_diffusion = st.slider(
        "拡散全体の寄与 w_diffusion",
        0.2, 3.0, float(p.w_diffusion), 0.05,
        help="Ds, Db, Dvに基づく緻密化全体の強さ。小さすぎると焼結が進みにくくなります。"
    )

    p.w_grain_growth = st.slider(
        "粒成長寄与 w_grain_growth",
        0.0, 3.0, float(p.w_grain_growth), 0.05,
        help="粒界移動による平均粒径の増加を調整します。"
    )

    p.w_liquid_phase = st.slider(
        "液相焼結寄与 w_liquid_phase",
        0.0, 3.0, float(p.w_liquid_phase), 0.05,
        help="Kingery型液相焼結、粒子再配列、溶解再析出の寄与を調整します。"
    )

    p.w_sintering_aid = st.slider(
        "焼結助剤寄与 w_sintering_aid",
        0.0, 3.0, float(p.w_sintering_aid), 0.05,
        help="助剤による拡散促進、液相形成、濡れ性改善の効果を調整します。"
    )

    p.w_second_phase_pinning = st.slider(
        "第二相ピン止め寄与 w_second_phase_pinning",
        0.0, 3.0, float(p.w_second_phase_pinning), 0.05,
        help="Zenerピン止めによる粒成長抑制の強さを調整します。"
    )

    p.w_atmosphere = st.slider(
        "雰囲気寄与 w_atmosphere",
        0.0, 3.0, float(p.w_atmosphere), 0.05,
        help="酸素分圧・水蒸気分圧による欠陥化学、粒界状態の補正を調整します。"
    )

    p.w_electric_field = st.slider(
        "電場・通電寄与 w_electric_field",
        0.0, 3.0, float(p.w_electric_field), 0.05,
        help="SPS/FAST/Flash焼結における電場促進やジュール発熱の寄与を調整します。"
    )

    p.w_closed_pore = st.slider(
        "閉気孔化による抑制 w_closed_pore",
        0.0, 3.0, float(p.w_closed_pore), 0.05,
        help="閉気孔化後の緻密化減速の強さを調整します。"
    )

    st.info(
        "調整の目安：液相焼結材料では w_liquid_phase と w_sintering_aid、"
        "ナノコンポジットでは w_second_phase_pinning、"
        "後期密度が合わない場合は w_closed_pore が特に重要です。"
    )
...
if 'last_df' not in st.session_state:
    ...
    with st.expander("物理モデル重み係数一覧を表示する", expanded=True):
        render_weight_guide(p)
...
with tab4:
    ...
    with st.expander("物理モデル重み係数一覧を表示する", expanded=False):
        render_weight_guide(p_show)
