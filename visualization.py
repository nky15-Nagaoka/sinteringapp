...
def _draw_initial_particles(ax, rng, n=46, title='(a) 粒子充填体'):
    ...
    ax.text(0.02,0.90,title, fontsize=13, weight='bold', bbox=dict(facecolor='white', edgecolor='black'))


def _draw_dense_polycrystal(ax, rng, rho=0.96, porosity=0.04, title='(b) 緻密化後'):
    ax.set_aspect('equal'); ax.set_xlim(0,1); ax.set_ylim(0,1); ax.axis('off')
    ...
    ax.text(0.02,0.90,title, fontsize=13, weight='bold', bbox=dict(facecolor='white', edgecolor='black'))
    ax.annotate('粒界', xy=(0.63,0.55), xytext=(0.78,0.67), color='white',
                arrowprops=dict(arrowstyle='->', color='white', lw=1.2), fontsize=11)
    if rho >= 0.97:
        ax.text(0.50,0.04,'ほぼ緻密化完了', ha='center', fontsize=10, bbox=dict(facecolor='white', edgecolor='black'))
    else:
        ax.text(0.50,0.04,'閉気孔が残存', ha='center', fontsize=10, bbox=dict(facecolor='white', edgecolor='black'))
...
def draw_transition_schematic(rho, porosity, seed=1):
    ...
    _draw_initial_particles(axes[0], rng, n=48, title='(a) 焼結前')
    ...
    axes[1].text(0.50,0.60,'焼結',ha='center',va='bottom',fontsize=12)
    _draw_dense_polycrystal(axes[2], rng, rho=rho, porosity=porosity, title='(b) 焼結後')
    fig.tight_layout()
    return fig
...
def draw_microstructure(rho, G_um, porosity, liquid_flag=0, seed=1):
    ...
    ax.set_ylim(0, 1)
    ax.axis('off')
    ax.set_title(f'現在の微細構造模式図  ρ={rho:.3f}, 気孔率={porosity:.3f}, G≈{G_um:.2f} µm')

    if rho < 0.70:
        _draw_initial_particles(ax, rng, n=44, title='初期: 粒子充填')
        ...
        ax.text(0.02,0.90,'中期: ネック成長・開気孔収縮', fontsize=11, weight='bold', bbox=dict(facecolor='white', edgecolor='black'))

    else:
        # From roughly closed-pore stage onward, draw as a dense polycrystal.
        _draw_dense_polycrystal(ax, rng, rho=rho, porosity=porosity, title='後期: 緻密多結晶')

    if liquid_flag:
        ax.text(0.02,0.03,'液相/助剤効果 ON', fontsize=10, bbox=dict(facecolor='white', edgecolor='black'))
    return fig
